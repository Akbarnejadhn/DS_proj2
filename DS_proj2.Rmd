...
geom_point(color = rgb(.2, .4, .2, .5)) +
    geom_line(aes(x = terminal, y = pred), data = ss_pred_df, color = "blue", size = 1) +
    ...
    geom_line(aes(x = terminal, y = pred), data = ss_pred_df30, color = "purple", size = 1) +
    theme_bw()

```

From the above model, we can observe that the degrees of freedom from GCV is `r round(ss_fit$df, 3)`, with the smoothing parameter of `r round(ss_fit2$spar, 3)`, and $\lambda$ of `r round(ss_fit2$lambda, 3)`.
