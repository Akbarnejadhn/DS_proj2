...
Some other candidates for non-linearity are the variables *apps*, *access*, *enroll*, *f_undergrad*, *p_undergrad*, *personal*, and *books*. However, identifying their trend need more investigation on their outlier values and cannot be determined only by looking at the plots.

So, there are 10 total number of candidates for non-linearity, and 3 non-linear variables.
...
It can be observed that the optimum number of parameters is `r mars_fit$bestTune`, so our finalModel will have 17 terms including the intercept.
I created partial dependence plots (PDPs) for some variables:
```{r PDP}

p1 = partial(mars_fit, pred.var = c("accept"), grid.resolution = 10) %>% autoplot()
p2 = partial(mars_fit, pred.var = c("enroll"), grid.resolution = 10) %>% autoplot()
p1
p2

p3 = partial(mars_fit, pred.var = c("accept", "enroll"), grid.resolution = 10) %>%
